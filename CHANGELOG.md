...
#### Fitur & Peningkatan Baru (New Features & Enhancements)
- `[JURNAL_KAS_PEMBUKUAN]` Membangun Sub-Modul Jurnal Pembukuan Kas Masuk & Kas Keluar (`financial_journals`):
  - **Pencatatan Arus Kas Masuk & Keluar:** Input penerimaan infaq/donasi (Transfer BSI, QRIS Sedekah Makan, Kotak Tunai, ZISWAF) dan pengeluaran operasional (Dapur, Kafalah, Sarpras, Listrik & Utilitas).
  - **Auto-Generator Kode Transaksi:** Format terstandar otomatis (`TRX-IN-xxx` untuk kas masuk dan `TRX-OUT-xxx` untuk kas keluar).
  - **Papan Metrik Neraca Kas:** Pemantauan real-time saldo kas bersih masjid, total kas masuk, total kas keluar, dan total volume transaksi.
  - **Ekspor Data Jurnal CSV:** Fitur ekspor seluruh data jurnal pembukuan ke format file CSV untuk audit pembukuan akuntansi terstandar.
  - **Unggah Bukti Struk/Kuitansi WebP:** Kompresi client-side otomatis bukti transfer dan nota bon dengan pratinjau lightbox.
- `[PENGAJUAN_ANGGARAN_BUDGET]` Membangun Sub-Modul Pengajuan Anggaran (*Budget Request*) & Reimbursement Nota Bon (`budget_requests`):
  - **Formulir Pengajuan Terbuka Seluruh Divisi:** Setiap PJ divisi dapat mengajukan permohonan dana kegiatan di muka atau klaim penggantian nota belanja.
  - **Panel Persetujuan Terpusat (*Approval Flow*):** Evaluasi khusus Ketua DKM & Bendahara untuk menyetujui (*Approve*), menolak (*Reject*), atau mencairkan dana (*Disburse*).
  - **Pencairan Otomatis ke Jurnal Kas:** Status *DISBURSED* secara otomatis mencatatkan pengeluaran baru ke tabel `financial_journals` tanpa perlu input manual ganda.
- `[SUBVIEW_SEGMENTASI]` Mengintegrasikan 3 Sub-View terorganisir pada tab `#tab-donations` (*Jurnal Kas Masuk & Keluar*, *Pengajuan Anggaran & Nota Bon*, dan *Infaq & Donasi Publik*).
