...
| 15 | Fitur Sakelar Status Live / Under Maintenance Terpadu & Halaman Pemeliharaan Kustom | `admin.html`, `maintenance.html`, `asset/js/env-loader.js` | **SELESAI (100% TERVERIFIKASI)** — Menyediakan sakelar status Live vs Maintenance di Visual Web Builder dengan proteksi wewenang (`SUPER_ADMIN`, `SUPER_USER`, `KETUA_DKM`, `PJ_MEDIA`), halaman kustom `maintenance.html` bertema islami elegan bebas emoji dengan kontak WhatsApp DKM & auto-reload realtime, intersepsi otomatis seluruh halaman publik di `env-loader.js`, serta bypass pratinjau khusus sesi pengurus DKM. |
